...
import { 
  ...
  getAllTestEmails, 
  rateLimitDelay 
} from '../data/test-vectors';
import {
  getTestClient,
  skipIfNotLive,
  skipIfNotComprehensive,
  RATE_LIMIT_DELAY
} from './setup';
      ...
      for (const testVector of TEST_VECTORS.proxy.ips) {
        try {
          ...
          if (ipData && typeof ipData === 'object') {
            ...
            console.log(`${testVector.value}: proxy=${ipData.proxy}, risk=${ipData.risk}%, type=${ipData.type}`);
          }
    ...
    it('should detect high-risk IPs from test vectors', async () => {
      ...
      for (const testVector of TEST_VECTORS.highRisk.ips) {
        ...
        if (ipData && typeof ipData === 'object' && typeof ipData.risk === 'number') {
          ...
          console.log(`High-risk IP ${testVector.value}: risk=${ipData.risk}%, proxy=${ipData.proxy}`);
        }
  ...
  describe('Email Validation', () => {
    it('should validate specific email test cases', async () => {
      ...
      for (const testCase of testCases) {
        ...
        if (emailData && typeof emailData === 'object') {
          expect(emailData).toHaveProperty('disposable');
          console.log(`${testCase.description} (${testCase.email}): disposable=${emailData.disposable}`);
    ...
    it('should track rate limit information', async () => {
      ...
      if (rateLimitInfo) {
        ...
        console.log('Rate limit info:', {
          ...
          reset: new Date((typeof rateLimitInfo.reset === 'number' ? rateLimitInfo.reset : 0) * 1000).toISOString()
        });
      }
    });
  });
