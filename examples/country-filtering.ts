...
async function countryBlockingExample() {
  ...
  try {
    for (const [label, ip] of Object.entries(testIPs)) {
      ...
      console.log(`  Country: ${addressData.country || 'Unknown'} (${addressData.isocode || 'N/A'})`);
...
async function geolocationAnalysisExample() {
  ...
  try {
    for (const [label, ip] of Object.entries(testIPs)) {
      ...
      if (result) {
        console.log(`  🌍 Location: ${result.city || 'Unknown'}, ${result.region || 'Unknown'}, ${result.country || 'Unknown'}`);
        ...
        console.log(`  💰 Currency: ${result.currency ? `${result.currency.name} (${result.currency.symbol})` : 'Unknown'}`);
